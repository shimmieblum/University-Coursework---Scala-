...
def clean(s: String) : List[String] = {
    val list = raw"\w+".r.findAllIn(s).toList
    list
}
...
def occurrences(xs: List[String]): Map[String, Int] = {
    val map = xs.groupBy(identity).view.mapValues(_.size).toMap
    map
}



//(3) This function calculates the dot-product of two documents
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    val result = map1.map{case(k,v) =>(k, v * map2.getOrElse(k,0))}.values.sum
    result
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val overlap:Double = prod(lst1,lst2).toDouble/Math.max(prod(lst1,lst1) , prod(lst2,lst2)).toDouble
    overlap
}

def similarity(s1: String, s2: String) : Double = {
    val result:Double = overlap(clean(s1), clean(s2))
    result
}
