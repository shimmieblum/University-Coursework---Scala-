...
def get_csv_url(url: String) : List[String] = {
     Source.fromURL(url, "UTF-8").getLines().toList.tail
...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    lines.map(_.split(",")).filter(x=>x(2).toInt > 3).map(x => (x(0),x(1)) )
}
...
def groupById(ratings: List[(String, String)], 
        ...
        if(ratings.isEmpty) m
            ...
            val movieID = ratings.head._2
            val newList = movieID::m.getOrElse(userID, List())
            groupById(ratings.tail, m+(userID->newList))
        }
    }
...
def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
    m.filter(_._2.contains(mov)).map(_._2.filter(x=> x != mov)).toList
}
...
favourites(ratings_map, "912").filter(x => x.contains("593")).length 
...
def suggestions(recs: Map[String, List[String]], 
               mov_name: String) : List[String] = {
                   favourites(recs,mov_name).flatten.groupBy(identity).view.mapValues(_.size).toList.sortBy(_._2).map(_._1).reverse
               }
...
def recommendations(recs: Map[String, List[String]],
                   ...
                   mov_name: String) : List[String] = {
                       suggestions(recs,mov_name).take(2).map(x=>movs(x))
                   }
