...
def jumpRight(prog: String, pc: Int, level: Int) : Int = if(prog.length == pc) pc else prog.charAt(pc) match {
...
def jumpLeft(prog: String, pc: Int, level: Int) : Int = if(pc < 0) pc else prog.charAt(pc) match {
      case '[' if(level == 0) => pc+1
...
def compute(prog: String, pc: Int, mp: Int, mem: Mem) : Mem = {
      // println("val: " +prog.charAt(pc)+"  pc: " + pc + "  mp: " + mp + "  mem: " +mem + "  mem read:" +sread(mem,mp))
      if(pc < 0 || pc == prog.length()) mem 
      else {
            val triple:(Int, Int, Mem) = prog.charAt(pc) match {
                  case '[' if(sread(mem, mp) == 0) => (jumpRight(prog, pc+1,0), mp, mem)
                  case '[' if(sread(mem, mp) != 0) => (pc+1, mp, mem)
                  case ']' if(sread(mem, mp) != 0) => (jumpLeft(prog, pc-1,0), mp, mem)
                  case ']' if(sread(mem,mp) == 0) => (pc+1, mp, mem)  
                  case '<' => (pc+1, mp-1, mem)
                  case '>' => (pc+1, mp+1, mem)
            ...
            compute(prog, triple._1, triple._2, triple._3)
      } 
}
